...
def generate_arc_map(data: pd.DataFrame, lat: float, lon: float, zoom: float, min_value=0, max_value=300, color_value=100) -> None:
    """
    Function to define PyDeck objects configurated for arc maps.
    It performs some pre-filtering.
    It then generates the two layers of the map. One for green connections, the other for red connections.
    It does not return any object, however it performs the streamlit writing function.
    """
    # pre filtering
    try:
